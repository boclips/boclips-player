...
responsibility to ensure that the token that is returned by `options.boclips.tokenFactory` is a valid token --
...
[WARNING]
====
`options.boclips.tokenFactory` is currently optional. In the v3.0.0 it will become mandatory.
====
