...
This is used if you have multiple boclips players on the page and only want to limit playback one at a time. When this is
on, and the user attempts to play a videos whilst another one is already playing.
The first one will stop, and the second video will start playing.
