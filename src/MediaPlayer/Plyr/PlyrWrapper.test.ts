...
describe('Addons', () => {
  it('initialises the addon when it can be enabled', () => {
    ...
    MockAddon.canBeEnabled.mockReturnValue(true);
  ...
  it('does not initialise the addon when it cannot be enabled', () => {
    ...
    MockAddon.canBeEnabled.mockReturnValue(false);
...
describe('Destruction', () => {
  ...
  it('destroys any enabled addons too', () => {
    ...
    const MockAddon = mocked(Addons[0]);
    const mockAddonInstance = MockAddon.mock.results[0].value;

    mediaPlayer.destroy();

    expect(mockAddonInstance.destroy).toHaveBeenCalled();
  });
});
