...
export default class PlyrWrapper implements MediaPlayer {
  ...
  public configureWithVideo = ({ playback }: Video) => {
    ...
    this.segment = this.player.getOptions().segment;
    ...
    Addons.forEach((AddonToInstall: Addon) => {
      if (
        ...
        const addonOptions: AddonOptions = {
          interface: this.getOptions(),
          segment: this.getSegment(),
        };

        // tslint:disable-next-line: no-unused-expression
        this.enabledAddons.push(
          new AddonToInstall(this.plyr, this.playback, addonOptions),
        );
      }
    });
  };
  ...
  public getSegment = () => this.player.getOptions().segment;
